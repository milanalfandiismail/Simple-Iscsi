...
pub async fn write_message(
    ...
    match msg {
        WriterMessage::Pdu(mut pdu) => {
            // Assign sequence numbers to target-to-initiator response PDUs
            pdu.cmd_sn = *stat_sn;
            *stat_sn = stat_sn.wrapping_add(1);
            pdu.exp_stat_sn = context.exp_cmd_sn.load(std::sync::atomic::Ordering::Relaxed);
            pdu.max_cmd_sn = context.max_cmd_sn.load(std::sync::atomic::Ordering::Relaxed);
            
            let packet = pdu::builder::build_pdu(&pdu);
            write_half.write_all(&packet).await?;
        }
        WriterMessage::R2T { itt, lun, buffer_offset, desired_len } => {
        ...
        WriterMessage::DataIn { itt, data, status, expected_len } => {
            let max_chunk = context.max_recv_data_segment_len;
            ...
            while offset < total_len {
                ...
                bhs[28..32].copy_from_slice(&context.exp_cmd_sn.load(std::sync::atomic::Ordering::Relaxed).to_be_bytes());
                bhs[32..36].copy_from_slice(&context.max_cmd_sn.load(std::sync::atomic::Ordering::Relaxed).to_be_bytes());
                ...
                if is_last {
                    ...
                    resp[24..28].copy_from_slice(&stat_sn.to_be_bytes());
                    *stat_sn = stat_sn.wrapping_add(1);
                    resp[28..32].copy_from_slice(&context.exp_cmd_sn.load(std::sync::atomic::Ordering::Relaxed).to_be_bytes());
                    resp[32..36].copy_from_slice(&context.max_cmd_sn.load(std::sync::atomic::Ordering::Relaxed).to_be_bytes());
        ...
        WriterMessage::CheckCondition { itt, key, asc, ascq } => {
...
impl SessionContext {
    pub(super) async fn send_r2t(
        ...
        let _ = self.tx.send(WriterMessage::R2T {
            itt,
            lun,
            buffer_offset,
            desired_len,
    ...
    pub(super) async fn send_scsi_response(
        ...
        let _ = self.tx.send(WriterMessage::ScsiResponse {
            itt,
            status,
            exp_data_sn,
            expected_len,
    ...
    pub(super) async fn send_scsi_check_condition(
        ...
        let _ = self.tx.send(WriterMessage::CheckCondition {
            itt,
            key,
            asc,
            ascq,
