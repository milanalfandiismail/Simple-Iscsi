...
def detect_projects_from_session(files_modified: list[dict], tool_calls: list[dict]) -> list[str]:
    ...
    workspace_root_str = str(WORKSPACE_ROOT).replace("\\", "/").replace("//", "/").lower()
    for p in all_paths:
        p_norm = p.replace("\\", "/").replace("//", "/").lower()
        if workspace_root_str in p_norm:
            relative = p_norm.split(workspace_root_str)[-1].lstrip("/")
