    ...
    for line in text.splitlines():
        ...
        if current_section == "tarefas pendentes":
            if stripped.startswith("- [ ]"):
                ...
                # Check for "(desde session-NNN)" and extract it
                source_session = 0
                match = re.search(r"\s*\(desde session-(\d+)\)", task_text)
                if match:
                    source_session = int(match.group(1))
                    task_text = task_text[:match.start()].strip()

                # De-duplicate: check if task already exists in list (case-insensitive)
                normalized_desc = task_text.strip().lower()
                existing_idx = -1
                for idx, t in enumerate(ctx.pending_tasks):
                    if t.description.strip().lower() == normalized_desc:
                        existing_idx = idx
                        break

                if existing_idx != -1:
                    # Keep the one with the smaller source_session (earlier session)
                    old_task = ctx.pending_tasks[existing_idx]
                    if source_session > 0 and (old_task.source_session == 0 or source_session < old_task.source_session):
                        old_task.source_session = source_session
                else:
