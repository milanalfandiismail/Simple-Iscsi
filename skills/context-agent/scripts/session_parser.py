...
def _parse_raw_entry(raw: dict) -> Optional[SessionEntry]:
    """Converte um dict JSON bruto em SessionEntry (suporta Claude Code e Antigravity IDE)."""
    # ── 1. Detecção do formato Antigravity IDE ───────────────────────
    if "source" in raw or raw.get("type") in ("USER_INPUT", "PLANNER_RESPONSE", "CODE_ACTION", "GREP_SEARCH", "VIEW_FILE"):
        source = raw.get("source", "")
        entry_type = raw.get("type", "")
        
        # Determinar papel (role)
        if entry_type == "USER_INPUT" or source == "USER_EXPLICIT":
            role = "user"
            mapped_type = "user"
        elif source == "MODEL" or entry_type in ("PLANNER_RESPONSE", "CODE_ACTION"):
            role = "assistant"
            mapped_type = "assistant"
        else:
            # Pula logs do sistema, grep, view_file para não duplicar conteúdo
            return None
        ...
        if isinstance(raw_calls, list):
            for tc in raw_calls:
                name = tc.get("name", "")
                args = tc.get("args", {})
                tool_calls.append({"name": name, "input": args, "args": args})
                
                # Detectar modificação de arquivo
                if name in ("replace_file_content", "multi_replace_file_content", "write_to_file"):
                    fp = args.get("TargetFile", "")
                    if not fp:
                        fp = args.get("target_file", "")
                    if fp:
        ...
        return SessionEntry(
            type=mapped_type,
            timestamp=timestamp,
            session_id=raw.get("sessionId", "session-antigravity"),
            slug=raw.get("slug", "antigravity"),
            role=role,
            content=content,
    ...
    # ── 2. Formato original do Claude Code ──────────────────────────
    entry_type = raw.get("type", "")
